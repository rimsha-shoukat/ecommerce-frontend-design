...
let listItems1 = productsList1.map((imgSrc) => {
    ...
    <a href="productDetails.html"> <img class="w-[15rem] p-2" src="${imgSrc}"/> </a>
       ...
       <span id="saveIcon" class="px-2 py-[0.3rem] border border-gray-300 shadow rounded-md" save=${imgSrc}>
       <i class="fa-regular fa-heart fa-lg cursor-pointer text-[#1491f0]"></i>
       </span>
    </div>`;
}).join(''); 
...
let listItems2 = productsList2.map((imgSrc) => {
    return `<div class="flex flex-col p-4 items-center justify-center bg-white rounded-md border border-gray-300 shadow gap-[2rem]">
        ...
        <div class="flex flex-row gap-4">
            ...
            <span id="saveIcon" class="w-[3rem] h-[2.5rem] p-2 border border-gray-300 shadow rounded-md" save=${imgSrc} >
              <i class="fa-regular fa-heart fa-lg cursor-pointer text-[#1491f0]"></i>
            </span>
        </div>
    </div>`;
