...
let moveToCart = document.getElementByClassName("moveToCart");
moveToCart.addEventListener("click", function(event){
  if(event.target.classList.contains("fa-hear")){
    const imgSrc = event.target.getAttribute('data-img');
    cart.push(imgSrc);
  }
})

let catItems = cart.map((item) =>{
...
let cartHtml = document.getElementByClassName("cart-items");
