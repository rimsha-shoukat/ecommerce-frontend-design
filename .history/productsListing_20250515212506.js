...
let listItems1 = productsList1.map((imgSrc) => {
    return `<div id="saveIcon" class="flex flex-row p-4 items-start justify-start bg-white rounded-md border border-gray-300 shadow gap-[1.5rem]">
    <a href="productDetails.html" class="itemDetail" detail="${imgSrc}"> <img class="w-[15rem] p-2" src="${imgSrc}"/> </a>
       <div class="flex flex-col w-[100%] gap-2 items-start justify-start">
         ...
         <p class="max-[760px]:hidde text-gray-700">Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit </p>
         <button class="cursor-pointer text-blue-600 hover:text-blue-800">View details</button>
       </div>
       <span class="saveBtn px-2 py-[0.3rem] border border-gray-300 shadow rounded-md cursor-pointer text-[#1491f0]" save="${imgSrc}">
...
let listItems2 = productsList2.map((imgSrc) => {
    return `<div id="saveIcon" class="flex flex-col p-4 items-center justify-center bg-white rounded-md border border-gray-300 shadow gap-[2rem]">
        <a href="productDetails.html" class="itemDetail" detail="${imgSrc}"> <img class="w-[15rem] p-2 h-[13rem]" src="${imgSrc}"/> </a>
        <div class="flex flex-row gap-4">
            ...
            <span class="saveBtn w-[3rem] h-[2.5rem] p-2 border border-gray-300 shadow rounded-md cursor-pointer text-[#1491f0]" save="${imgSrc}" >
              <i class="fa-regular fa-heart fa-lg"></i>
            </span>
        </div>
    </div>`;
