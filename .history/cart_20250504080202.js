        ...
        let cartHtml = item.map((i) =>{
            return `<span class="flex flex-row items-start justify-start gap-4 border-b p-4 border-gray-300 w-[100%]">
                        ...
                        <span class="flex flex-col items-start justify-start">
                            ...
                            <span id="updateCart" class="flex flex-row gap-6 mt-4">
                                ...
                                <button save="${i}" class="save px-2 py-[0.3rem] border border-gray-300 cursor-pointer shadow rounded-md text-blue-500 hover:bg-blue-500 hover:text-white font-semibold">Save for later</button>
                            </span>
                        </span>
...
saves.addEventListener("click", (event) => {
    if(event.target.closest(".moveToCart")){
        ...
        if (!cart.includes(newItem)) {
            ...
            localStorage.setItem("Cart", JSON.stringify(cart));
            localStorage.setItem('saconst index = saveItems.indexOf(newItem);
            if (index > -1) {
                saveItems.splice(index, 1);
            }veItems', JSON.stringify(saveItems));
            moved(cart);
